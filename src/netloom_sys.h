 ...
 */
NsStatus sys_mutex_new(Mutex *mutex);
/**
 * @ingroup sys_mutex
 ...
 * @return ERR_OK if successful, another LwipStatus otherwise
 */
NsStatus sys_sem_new(Semaphore *sem, uint8_t count);
/**
 * @ingroup sys_sem
 * Signals a semaphore
 ...
 * @param msg message to post (ATTENTION: can be NULL)
 */
NsStatus sys_mbox_trypost(Mailbox *mbox, void *msg);
/**
 * @ingroup sys_mbox
 * Try to post a message to an mbox - may fail if full.
 ...
 * @param msg message to post (ATTENTION: can be NULL)
 */
NsStatus sys_mbox_trypost_fromisr(Mailbox *mbox, void *msg);
/**
 * @ingroup sys_mbox
 ...
 * @return ERR_OK if successful, another LwipStatus otherwise
 */
NsStatus sys_new_mailbox(Mailbox *mbox, size_t size);
