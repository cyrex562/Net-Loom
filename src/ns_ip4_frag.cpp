...
struct IpReassHelper {
    struct PacketContainer* next_pbuf;
    uint16_t start;
    uint16_t end;
};
...
{
    return
        (&(iphdr_a)->src.u32 &(iphdr_b)->src.u32) && (&(iphdr_a)->dest.u32 &(iphdr_b)->dest.u32) &&
...
ip_reass_free_complete_datagram(struct ip_reassdata *ipr, struct ip_reassdata *prev)
{
  ...
  ns_assert("prev != ipr", prev != ipr);
  if (prev != nullptr) {
    ns_assert("prev->next == ipr", prev->next == ipr);
  }
  ...
  if (iprh->start == 0) {
    ...
    // clen = pbuf_clen(p);
    ns_assert("pbufs_freed + clen <= 0xffff", pbufs_freed + clen <= 0xffff);
  ...
  ns_assert("ip_reass_pbufcount >= pbufs_freed", ip_reass_pbufcount >= pbufs_freed);
...
ip_reass_dequeue_datagram(struct ip_reassdata *ipr, struct ip_reassdata *prev)
{
  ...
  if (reassdatagrams == ipr) {
    ...
    ns_assert("sanity check linked list", prev != nullptr);
    prev->next = ipr->next;
  }
 ...
 */
static int
ip_reass_chain_frag_into_datagram_and_validate(struct ip_reassdata *ipr, struct PacketContainer *new_p, int is_last)
{
  ...
  uint16_t len = ns_ntohs(get_ip4_hdr_len(fraghdr));
  ...
  ns_assert("sizeof(struct ip_reass_helper) <= IP_HLEN",
  ...
  if (q == nullptr) {
    if (iprh_prev != nullptr) {
      ...
      ns_assert("check fragments don't overlap", iprh_prev->end <= iprh->start);
      ...
      ns_assert("no previous fragment, this must be the first fragment!",
                  ipr->p == nullptr);
  ...
  if (is_last || ((ipr->flags & kIpReassFlagLastfrag) != 0)) {
    ...
    if (valid) {
      ...
      if ((ipr->p == nullptr) || (((struct IpReassHelper *)ipr->p->payload)->start != 0)) {
        ...
        if (valid) {
          ns_assert("sanity check", ipr->p != nullptr);
          ns_assert("sanity check",
                      ((struct IpReassHelper *)ipr->p->payload) != iprh);
          ns_assert("validate_datagram:next_pbuf!=NULL",
                      iprh->next_pbuf == nullptr);
        }
      }
    }
...
ip4_reass(struct PacketContainer *p)
{
...
//                                   lwip_ntohs(IPH_ID(fraghdr))));
  ...
  if (ipr == nullptr) {
    ...
    if (((ns_ntohs(get_ip4_hdr_offset(fraghdr)) & IP4_OFF_MASK) == 0) &&
        ((ns_ntohs(get_ip4_hdr_offset(&ipr->iphdr)) & IP4_OFF_MASK) != 0)) {
...
//                 ipr->datagram_len));
  ...
  if (valid == IP_REASS_VALIDATE_TELEGRAM_FINISHED) {
    ...
    struct PacketContainer* r = ((struct IpReassHelper *)ipr->p->payload)->next_pbuf;
    ...
    set_ip4_hdr_len(fraghdr, ns_htons(datagram_len));
    ...
    // clen = pbuf_clen(p);
    ns_assert("ip_reass_pbufcount >= clen", ip_reass_pbufcount >= clen);
    ip_reass_pbufcount = (uint16_t)(ip_reass_pbufcount - clen);
...
nullreturn_ipr:
  ns_assert("ipr != NULL", ipr != nullptr);
  if (ipr->p == nullptr) {
    /* dropped PacketBuffer after creating a new datagram entry: remove the entry, too */
    ns_assert("not firstalthough just enqueued", ipr == reassdatagrams);
 ...
 * free_pkt_buf. */
static void
ipfrag_free_pbuf_custom(struct PacketContainer *p)
{
  struct PbufCustomRef *pcr = (struct PbufCustomRef *)p;
  ns_assert("pcr != NULL", pcr != nullptr);
  ns_assert("pcr == p", (uint8_t *)pcr == (uint8_t *)p);
 ...
 */
NsStatus
ip4_frag(PacketContainer& pkt_buf, NetworkInterface& netif, const Ip4Addr& dst_addr)
{
  struct PacketContainer rambuf{};
  ...
  uint16_t tmp = ns_ntohs(get_ip4_hdr_offset(iphdr));
  ...
  while (left) {
    ...
    ns_assert("this needs a PacketBuffer in one piece!",
    ...
    while (left_to_copy) {
      ...
      ns_assert("p->len >= poff", pkt_buf->len >= poff);
    ...
    set_ip4_hdr_offset(iphdr, ns_htons(tmp));
    set_ip4_hdr_len(iphdr, ns_htons((uint16_t)(fragsize + IP4_HDR_LEN)));
