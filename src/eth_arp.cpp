 ...
 */
bool
etharp_recv(PacketContainer& pkt_buf,
            NetworkInterface& netif,
            DhcpContext& ctx,
            std::vector<EtharpEntry>& entries)
{
    const Ip4Addr src_ip_addr{};
    ...
    if ((hdr.hwtype != pp_htons(LWIP_IANA_HWTYPE_ETHERNET)) || (hdr.hwlen != MAC_ADDR_LEN)
     ...
     */
    // todo: move code to another location?
    // autoip_arp_reply(netif, hdr, auto_ip_state);
    ...
    */
    ...
    etharp_update_arp_entry(netif, src_ip_addr, hdr.shwaddr, false, entries);
    ...
    if (hdr.opcode == pp_htons(ARP_REQUEST)) {
         ...
         */
        if (!dhcp_arp_reply(netif, src_ip_addr, ctx)) { return false; }
    }
    else {
             ...
             std::vector<EtharpEntry>& entries)
{
    ...
    if (arp_table[i].state >= ETHARP_STATE_STABLE) {
        ...
        struct PacketContainer* p = pkt;
        while (p) {
            ns_assert("no packet queues allowed!",
 ...
 *
 * @param netif the ns network interface on which to send the ARP packet
                 ...
                 const uint16_t opcode)
{
    ...
    PacketContainer packet_buffer{};
    EtharpHdr etharp_hdr{};
    etharp_hdr.opcode = ns_htons(opcode);
 ...
 * times out
 *
 * @param netif the ns network interface on which to send the request
