 ...
 */
bool
bridgeif_output(NetworkInterface& netif, PacketContainer& pkt_buf, BridgeInterface& bridge_ifc)
{
 ...
 */
static bool
bridgeif_input(PacketContainer& pkt_buf,
 ...
 *
 * @param ifc the ns network interface structure for this ethernetif
...
bridgeif_init(NetworkInterface& ifc, BridgeIfcInitData& init_data)
{
    ...
    // lwip_assert("init_data != NULL", (init_data != nullptr));
    ns_assert("init_data->max_ports <= BRIDGEIF_MAX_PORTS",
                init_data.max_ports <= BRIDGE_IFC_MAX_PORTS);
