...
static int b64enc(bs, inp, inlen, outp) struct b64state *bs;
uint8_t *inp;
int inlen;
uint8_t *outp;
{
...
static int b64flush(bs, outp) struct b64state *bs;
uint8_t *outp;
{
  int outlen = 0;
...
static int b64dec(bs, inp, inlen, outp) struct b64state *bs;
uint8_t *inp;
int inlen;
uint8_t *outp;
{
 ...
 */
static void eap_figure_next_state(PppPcb *pcb, int status) {
...
static void eap_send_request(PppPcb *pcb) {
...
#ifdef USE_SRP
  struct t_server *ts;
  uint8_t clear[8], cipher[8], dig[SHA_DIGESTSIZE], *optr, *cp;
  int i, j;
  struct b64state b64;
  SHA1_CTX ctxt;
#endif /* USE_SRP */
  ...
  p = pbuf_alloc(PBUF_RAW, (uint16_t)(PPP_CTRL_PBUF_MAX_SIZE),
                 PPP_CTRL_PBUF_TYPE);
  if (nullptr == p) return;
  if (p->tot_len != p->len) {
    pbuf_free(p);
  ...
  switch (pcb->eap.es_server.ea_state) {
    ...
    case eapSRP3:
      ...
      if (pncrypt_setkey(0)) {
        ...
        if ((j = i = pcb->eap.es_server.ea_peerlen) > 7) j = 7;
        clear[0] = i;
        MEMCPY(clear + 1, cp, j);
        i -= j;
        cp += j;
        ...
        *optr = i;
        ...
        while (optr < outp) {
          SHA1Final(dig, &ctxt);
          cp = dig;
          while (cp < dig + SHA_DIGESTSIZE) *optr++ ^= *cp++;
          SHA1Init(&ctxt);
          SHA1Update(&ctxt, &pcb->eap.es_server.ea_id, 1);
          SHA1Update(&ctxt, pcb->eap.es_server.ea_skey, SESSION_KEY_LEN);
 ...
 */
static void eap_send_response(PppPcb *pcb, uint8_t id, uint8_t typenum,
                              const uint8_t *str, int lenstr) {
  struct PacketBuffer *p;
  uint8_t *outp;
  int msglen;
 ...
 */
static void eap_chap_response(PppPcb *pcb, uint8_t id, uint8_t *hash,
                              const char *name, int namelen) {
  struct PacketBuffer *p;
  uint8_t *outp;
  int msglen;

  msglen = EAP_HEADERLEN + 2 * sizeof(uint8_t) + MD5_SIGNATURE_SIZE + namelen;
  p = pbuf_alloc(PBUF_RAW, (uint16_t)(PPP_HDRLEN + msglen), PPP_CTRL_PBUF_TYPE);
  if (nullptr == p) return;
  if (p->tot_len != p->len) {
    pbuf_free(p);
...
int lenstr;
{
  ...
  msglen = EAP_HEADERLEN + 2 * sizeof(uint8_t) + lenstr;
  p = pbuf_alloc(PBUF_RAW, (uint16_t)(PPP_HDRLEN + msglen), PPP_CTRL_PBUF_TYPE);
  if (NULL == p) return;
  if (p->tot_len != p->len) {
    pbuf_free(p);
...
uint8_t *str;
{
  ...
  msglen =
      EAP_HEADERLEN + 2 * sizeof(uint8_t) + sizeof(uint32_t) + SHA_DIGESTSIZE;
  p = pbuf_alloc(PBUF_RAW, (uint16_t)(PPP_HDRLEN + msglen), PPP_CTRL_PBUF_TYPE);
  if (NULL == p) return;
  if (p->tot_len != p->len) {
    pbuf_free(p);
...
static int open_pn_file(modebits) mode_t modebits;
{
  char *path;
  int fd, err;

  if ((path = name_of_pn_file()) == NULL) return (-1);
...
int len, id;
{
  ...
  val = id;
  while (len > 0) {
    if ((dsize = len % SHA_DIGESTSIZE) == 0) dsize = SHA_DIGESTSIZE;
    len -= dsize;
    datp = inp + len;
    SHA1Init(&ctxt);
    ...
    SHA1Final(dig, &ctxt);
    for (digp = dig; digp < dig + SHA_DIGESTSIZE; digp++) *datp++ ^= *digp;
  }
 ...
 */
static void eap_request(PppPcb *pcb, uint8_t *inp, int id, int len) {
  uint8_t typenum;
  uint8_t vallen;
  int secret_len;
  ...
  switch (typenum) {
    case EAPT_IDENTITY:
      ...
      if (pcb->eap.es_usepseudo &&
        ...
        if ((fd = open_pn_file(O_RDONLY)) >= 0) {
          ...
          /* XXX NAI unsupported */
          if (len > 0) {
            eap_send_response(pcb, id, typenum, rhostname,
                              len + SRP_PSEUDO_LEN);
          }
          (void)close(fd);
          if (len > 0) break;
      ...
      eap_send_response(pcb, id, typenum,
                        (const uint8_t *)pcb->eap.es_client.ea_name,
                        pcb->eap.es_client.ea_namelen);
      break;
    ...
    case EAPT_MD5CHAP:
      ...
      lwip_md5_update(&mdContext, (uint8_t *)secret, secret_len);
      ...
      switch (vallen) {
        ...
        case EAPSRP_SVALIDATOR:
          ...
          if (pcb->eap.es_client.ea_state == eapOpen) {
            ...
            if (len < 0 || t_clientverify(tc, inp + sizeof(uint32_t)) != 0) {
              ppp_error(
 ...
 */
static void eap_response(PppPcb *pcb, uint8_t *inp, int id, int len) {
  uint8_t typenum;
  uint8_t vallen;
  int secret_len;
  char secret[MAXSECRETLEN];
