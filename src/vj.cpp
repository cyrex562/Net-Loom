...
vj_compress_tcp(struct VjCompress* comp, struct PacketBuffer** pb)
{
   ...
   */ // vjs_packets +=1;
    if (!ip4_addr_cmp(&ip->src, &cs->vjcs_u.csu_ip.src) || !
        ip4_addr_cmp(&ip->dest, &cs->vjcs_u.csu_ip.dest) || (*(struct vj_uint32_t*)th).v
            ...
            &cs->vjcs_u.csu_ip)]).v)
    {
        ...
        do
        {
            ...
            if (ip4_addr_cmp(&ip->src, &cs->vjcs_u.csu_ip.src) &&
                ip4_addr_cmp(&ip->dest, &cs->vjcs_u.csu_ip.dest) && (*(struct vj_uint32_t*
     ...
     */ // INCR(vjs_packets);
        if (!ip4_addr_cmp(&ip->src, &cs->vjcs_u.csu_ip.src) || !
            ip4_addr_cmp(&ip->dest, &cs->vjcs_u.csu_ip.dest) || (*(struct vj_uint32_t*)th)
                ...
                &cs->vjcs_u.csu_ip)]).v)
        {
            ...
            do
            {
                ...
                cs = cs->cs_next; // INCR(vjs_searches);
                if (ip4_addr_cmp(&ip->src, &cs->vjcs_u.csu_ip.src) &&
                    ip4_addr_cmp(&ip->dest, &cs->vjcs_u.csu_ip.dest) && (*(struct
...
vj_uncompress_tcp(struct PacketBuffer** nb, struct VjCompress* comp)
{
    ...
    case SPECIAL_I:
        {
            uint32_t i = lwip_ntohs(get_ip4_hdr_len(&cs->vjcs_u.csu_ip)) - cs->cs_hlen;
    ...
    case SPECIAL_D: /* some compilers can't nest inline assembler.. */ tmp =
            lwip_ntohl(th->seqno) + lwip_ntohs(get_ip4_hdr_len(&cs->vjcs_u.csu_ip)) - cs->
            cs_hlen;
        th->seqno = lwip_htonl(tmp);
        break;
    default:
    ...
    else
    {
        set_ip4_hdr_id(&cs->vjcs_u.csu_ip,
                       lwip_ntohs(get_ip4_hdr_id(&cs->vjcs_u.csu_ip)) + 1);
        set_ip4_hdr_id(&cs->vjcs_u.csu_ip,
                       lwip_htons(get_ip4_hdr_id(&cs->vjcs_u.csu_ip)));
    }
    if (changes & NEW_I)
    {
        // DecodeShort(cs->vjcs_u.csu_ip._id);
 ...
 */
    ...
    if (n0->len < vjlen)
    {
        ...
        set_ip4_hdr_len(&cs->vjcs_u.csu_ip, lwip_htons((uint16_t)tmp));
#else
