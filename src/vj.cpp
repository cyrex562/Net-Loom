  ...
  if (!ip4_addr_cmp(&ip->src, &cs->cs_ip.src)
    ...
    *pb = np;
    ...
    case 0:
         ...
         */
        if (get_ip4_hdr_len(ip) != get_ip4_hdr_len(&cs->cs_ip) &&
            lwip_ntohs(get_ip4_hdr_len(&cs->cs_ip)) == hlen)
        {
            break;
        }
        /* no break */
    ...
    case NEW_S | NEW_A:
        if (deltaS == deltaA && deltaS == lwip_ntohs(get_ip4_hdr_len(&cs->cs_ip)) - hlen)
        {
    ...
    case NEW_S:
        if (deltaS == lwip_ntohs(get_ip4_hdr_len(&cs->cs_ip)) - hlen)
        {
    ...
    deltaS = (uint16_t)(lwip_ntohs(get_ip4_hdr_id(ip)) - lwip_ntohs(get_ip4_hdr_id(&cs->cs_ip)));
    if (deltaS != 1)
    {
        Encodez(deltaS);
        changes |= NEW_I;
...
vj_uncompress_tcp(struct PacketBuffer** nb, struct vjcompress* comp)
{
    ...
    case SPECIAL_I:
        {
            uint32_t i = lwip_ntohs(get_ip4_hdr_len(&cs->cs_ip)) - cs->cs_hlen;
    ...
    case SPECIAL_D:
        /* some compilers can't nest inline assembler.. */
        tmp = lwip_ntohl(th->seqno) + lwip_ntohs(get_ip4_hdr_len(&cs->cs_ip)) - cs->cs_hlen;
        th->seqno = lwip_htonl(tmp);
        break;
    ...
    {
        IPH_ID_SET(&cs->cs_ip, lwip_ntohs(get_ip4_hdr_id(&cs->cs_ip)) + 1);
        IPH_ID_SET(&cs->cs_ip, lwip_htons(get_ip4_hdr_id(&cs->cs_ip)));
    }
    break;
  }
  if (changes & NEW_I) {
