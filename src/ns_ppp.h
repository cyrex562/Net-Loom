...
bool ppp_input(PppPcb& ppp_pcb, PacketContainer& pkt_buf, Fsm& lcp_fsm);
...
bool
ppp_write(PppPcb& pcb, PacketContainer& p);
...
{
    ((ppp).ipcp_allowoptions.dnsaddr[index] = (addr.u32));
}
 ...
 */
NsStatus ppp_connect(PppPcb& pcb, uint64_t holdoff);
 ...
 * established before calling this.
 */
NsStatus ppp_listen(PppPcb& pcb);
 ...
 */
NsStatus ppp_close(PppPcb& pcb, bool nocarrier);
 ...
 * Return 0 on success, an error code on failure.
 */
NsStatus ppp_ioctl(PppPcb *pcb, uint8_t cmd, uint8_t *arg);
...
static NsStatus ppp_netif_init_cb(NetworkInterface* netif);

static NsStatus ppp_netif_output_ip4(NetworkInterface& netif, PacketContainer& pb, const Ip4Addr& ipaddr, PppPcb& ppp_pcb);

static NsStatus ppp_netif_output_ip6(NetworkInterface& netif, PacketContainer& pb, const Ip6Addr& ipaddr, PppPcb& ppp_pcb);

NsStatus
ppp_netif_output(NetworkInterface& netif, PacketContainer& pb, PppProtoFieldValue protocol, PppPcb& ppp_pcb);
