...
get_tcp_hdr_len(TcpHdr* tcp_hdr, const bool get_bytes = false)
{
    if (get_bytes)
    {
        return (ns_ntohs(tcp_hdr->_hdrlen_rsvd_flags) >> 12) << 2;
    }
    return ns_ntohs(tcp_hdr->_hdrlen_rsvd_flags) >> 12;
} ///
///
///
inline uint8_t
...
{
    return uint8_t((ns_ntohs((phdr)->_hdrlen_rsvd_flags) & TCP_FLAGS));
} ///
///
///
inline void
...
{
    (phdr)->_hdrlen_rsvd_flags = ns_htons(((len) << 12) | tcph_flags(phdr));
}
...
{
    (phdr)->_hdrlen_rsvd_flags = uint16_t(ns_htons(uint16_t((len) << 12) | (flags)));
}

inline void
TCPH_SET_FLAG(TcpHdr* phdr, uint8_t flags)
{
    (phdr)->_hdrlen_rsvd_flags = ((phdr)->_hdrlen_rsvd_flags | ns_htons(flags));
}
...
/** Function prototype for tcp accept callback functions. Called when a new
      ...
      */
typedef NsStatus
(*tcp_accept_fn)(void* arg, struct TcpPcb* newpcb, NsStatus err);
/** Function prototype for tcp receive callback functions. Called when data has
      ...
      */
typedef NsStatus
(*tcp_recv_fn)(void* arg, struct TcpPcb* tpcb, struct PacketContainer* p, NsStatus err);
...
/** Function prototype for tcp error callback functions. Called when the pcb
      ...
      */
typedef void
(*tcp_err_fn)(void* arg, NsStatus err);
/** Function prototype for tcp connected callback functions. Called when a pcb
      ...
      */
typedef NsStatus
(*tcp_connected_fn)(void* arg, struct TcpPcb* tpcb, NsStatus err);
