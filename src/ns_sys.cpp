...
sys_msleep(const uint32_t ms)
{
  if (ms > 0) {
    Semaphore delaysem{};
    NsStatus err = sys_sem_new(&delaysem, 0);
...
uint32_t sys_win_rand()
{
    ...
    ns_assert("CryptGenRandom failed", false);
    return 0;
}
...
msvc_sys_init()
{
    ...
    ns_assert("TlsAlloc failed", netconn_sem_tls_index != TLS_OUT_OF_INDEXES);
}
...
sys_sem_new(Semaphore* sem, uint8_t count)
{
    ...
    ns_assert("sem != NULL", sem != nullptr);

    new_sem = CreateSemaphore(nullptr, count, 100000, nullptr);
    ns_assert("Error creating semaphore", new_sem != nullptr);
    if (new_sem != nullptr)
    {
        if (sys_initialized())
        {
...
sys_sem_free(Semaphore* sem)
{
    /* parameter check */
    ns_assert("sem != NULL", sem != nullptr);
    ns_assert("sem->sem != NULL", sem->sem != nullptr);
    ns_assert("sem->sem != INVALID_HANDLE_VALUE", sem->sem != INVALID_HANDLE_VALUE);
    CloseHandle(sem->sem);
...
sys_arch_sem_wait(Semaphore *sem, uint32_t timeout)
{
  ...
  if (!timeout) {
    ...
    ns_assert("Error waiting for semaphore", ret == WAIT_OBJECT_0);
    ...
    ns_assert("Error waiting for semaphore", (ret == WAIT_OBJECT_0) || (ret == WAIT_TIMEOUT));
...
sys_sem_signal(Semaphore *sem)
{
  ...
  ns_assert("Error releasing semaphore", ret != 0);
}
...
sys_mutex_new(Mutex* mutex)
{
    ...
    ns_assert("mutex != NULL", mutex != nullptr);

    new_mut = CreateMutex(nullptr, FALSE, nullptr);
    ns_assert("Error creating mutex", new_mut != nullptr);
    if (new_mut != nullptr)
    {
        do
        {
...
sys_mutex_free(Mutex* mutex)
{
    /* parameter check */
    ns_assert("mutex != NULL", mutex != nullptr);
    ns_assert("mutex->mut != NULL", mutex->mut != nullptr);
    ns_assert("mutex->mut != INVALID_HANDLE_VALUE", mutex->mut != INVALID_HANDLE_VALUE);
...
void sys_mutex_lock(Mutex* mutex)
{
    ...
    ns_assert("Error waiting for mutex", ret == WAIT_OBJECT_0);
}
...
sys_mutex_unlock(Mutex* mutex)
{
    ...
    {
        ns_assert("Error releasing mutex", false);
    }
}
...
sys_thread_new(const char* name, LwipThreadFn function, void* arg, int stacksize, int prio, ThreadList* thread_list)
{
    ...
    ns_assert("new_thread != NULL", new_thread != nullptr);
    if (new_thread != nullptr)
    {
        ...
        ns_assert("h != 0", h != nullptr);
        ns_assert("h != -1", h != INVALID_HANDLE_VALUE);
...
{
    ns_assert("mbox != NULL", mbox != nullptr);
    mbox->sem = CreateSemaphore(nullptr, 0, MAX_QUEUE_ENTRIES, nullptr);
    ns_assert("Error creating semaphore", mbox->sem != nullptr);
    if (mbox->sem == nullptr)
    {
        sys_prot_t lev;
        SYS_ARCH_PROTECT(lev);
...
sys_free_mailbox(Mailbox* mbox)
{
    /* parameter check */
    ns_assert("mbox != NULL", mbox != nullptr);
    ns_assert("mbox->sem != NULL", mbox->sem != nullptr);
    ns_assert("mbox->sem != INVALID_HANDLE_VALUE", mbox->sem != INVALID_HANDLE_VALUE);
...
sys_mbox_post(Mailbox* mbox, void* msg)
{
    ...
    /* parameter check */
    ns_assert("q != SYS_MBOX_NULL", mbox != nullptr);
    ns_assert("q->sem != NULL", mbox->sem != nullptr);
    ns_assert("q->sem != INVALID_HANDLE_VALUE", mbox->sem != INVALID_HANDLE_VALUE);
    ...
    ns_assert("mbox is full!", mbox->head != mbox->tail);
    const auto ret = ReleaseSemaphore(mbox->sem, 1, nullptr);
    ns_assert("Error releasing sem", ret != 0);
...
sys_mbox_trypost(Mailbox* q, void* msg)
{
    ...
    /* parameter check */
    ns_assert("q != SYS_MBOX_NULL", q != nullptr);
    ns_assert("q->sem != NULL", q->sem != nullptr);
    ns_assert("q->sem != INVALID_HANDLE_VALUE", q->sem != INVALID_HANDLE_VALUE);
    ...
    q->q_mem[q->head] = msg;
    q->head = new_head;
    ns_assert("mbox is full!", q->head != q->tail);
...
sys_arch_mbox_fetch(Mailbox* q, void** msg, uint32_t timeout)
{
    ...
    {
        ns_assert("Error waiting for sem", ret == WAIT_TIMEOUT);
        if (msg != nullptr)
        {
            *msg = nullptr;
        }
...
void sys_arch_netconn_sem_alloc()
{
    ...
    NsStatus err = sys_sem_new(sem, 0);
