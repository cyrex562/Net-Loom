 ...
 */
void
udp_input(struct PacketContainer* p, NetworkInterface* inp)
{
    ...
    uint8_t for_us = 0;
    ns_assert("udp_input: invalid pbuf", p != nullptr);
    ns_assert("udp_input: invalid netif", inp != nullptr); // UDP_STATS_INC(udp.recv);
    ...
    uint16_t src = ns_ntohs(udphdr->src);
    uint16_t dest = ns_ntohs(udphdr->dest);
    ...
    if (for_us)
    {
        ...
        if (is_netif_checksum_enabled(inp, NETIF_CHECKSUM_CHECK_UDP))
        {
            if (curr_proto == IP_PROTO_UDPLITE)
            {
                ...
                uint16_t chklen = ns_ntohs(udphdr->len);
 ...
 */
NsStatus
udp_send(struct UdpPcb* pcb, struct PacketContainer* p)
{
 ...
 */
NsStatus
udp_send_chksum(UdpPcb* pcb, struct PacketContainer* p, uint8_t have_chksum, uint16_t chksum)
{
    if ((pcb->remote_ip.type == IP_ADDR_TYPE_ANY))
    {
        return ERR_VAL;
 ...
 */
bool
udp_sendto(UdpPcb& pcb, PacketContainer& p, const IpAddrInfo& dst_ip, const uint16_t dst_port)
{
...
{
    NsStatus err;
    struct PacketContainer q{}; /* q will be sent down the stack */
    uint8_t ip_proto;
    ...
    // }
    ns_assert("check that first PacketBuffer can hold UdpHdr",
                (q->len >= sizeof(struct UdpHdr)));
    /* q now represents the packet to be sent */
    struct UdpHdr* udphdr = (struct UdpHdr *)q->payload;
    udphdr->src = ns_htons(pcb->local_port);
    udphdr->dest = ns_htons(dst_port); /* in UDP, 0 checksum means 'no checksum' */
    ...
    if (pcb->flags & UDP_FLAGS_UDPLITE)
    {
        ...
        udphdr->len = ns_htons(chklen_hdr); /* calculate checksum */
    ...
    else
    {
        ...
        udphdr->len = ns_htons(q->tot_len); /* calculate checksum */
        if(is_netif_checksum_enabled(netif, NETIF_CHECKSUM_GEN_UDP))
        {
...
udp_connect(struct UdpPcb* pcb, const IpAddrInfo* ipaddr, uint16_t port)
{
    if (pcb->local_port == 0)
    {
        NsStatus err = udp_bind(pcb, &pcb->local_ip, pcb->local_port);
        if (err != STATUS_SUCCESS)
        {
            return err;
        }
