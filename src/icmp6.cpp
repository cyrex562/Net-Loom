...
///
void icmp6_send_response(PacketContainer& p,
                                uint8_t code,
                                uint32_t data,
                                uint8_t type);

///
void icmp6_send_response_with_addrs(PacketContainer& p,
                                           uint8_t code,
                                           uint32_t data,
                                           uint8_t type,
...
///
void icmp6_send_response_with_addrs_and_netif(struct PacketContainer* p,
                                                     uint8_t code,
                                                     uint32_t data,
                                                     uint8_t type,
                                                     const Ip6Addr* src_addr,
...
///
void icmp6_input(PacketContainer& pkt_buf, NetworkInterface& netif)
{
    struct PacketContainer r{};
 ...
 */
void icmp6_dest_unreach(struct PacketContainer* p, enum icmp6_dur_code c)
{
 ...
 */
void icmp6_packet_too_big(PacketContainer& p, uint32_t mtu)
{
 ...
 */
void icmp6_time_exceeded(PacketContainer& p, enum Icmp6TeCode c)
{
 ...
 *                  information
 */
void icmp6_time_exceeded_with_addrs(PacketContainer& p,
                                    enum Icmp6TeCode c,
                                    const Ip6Addr& src_addr,
                                    const Ip6Addr& dest_addr)
{
 ...
 */
void icmp6_param_problem(struct PacketContainer* p,
 ...
 */
static void icmp6_send_response(PacketContainer& p,
                                uint8_t code,
                                uint32_t data,
                                uint8_t type)
{
    NetworkInterface* netif = nullptr;
    ns_assert("icmpv6 packet not a direct response", netif != nullptr);
    const Ip6Addr* reply_dest = nullptr; /* Select an address to use as source. */
    Ip6Addr reply_src = netif_select_ip6_src_addr(netif, reply_dest)->u_addr.ip6;
 ...
 */
static void icmp6_send_response_with_addrs(PacketContainer& p,
                                           uint8_t code,
                                           uint32_t data,
                                           uint8_t type,
                                           const Ip6Addr& src_addr,
 ...
 */
static void icmp6_send_response_with_addrs_and_netif(struct PacketContainer* p,
                                                     uint8_t code,
                                                     uint32_t data,
                                                     uint8_t type,
