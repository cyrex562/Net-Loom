 ...
 */
typedef void (* sys_timeout_handler)(uint8_t *arg);
...
void sys_timeout_debug(uint32_t msecs, sys_timeout_handler handler, uint8_t *arg, const char* handler_name);
#define sys_timeout(msecs, handler, arg) sys_timeout_debug(msecs, handler, arg, #handler)


void sys_untimeout(sys_timeout_handler handler, uint8_t *arg);
void sys_restart_timeouts(void);
void sys_check_timeouts(void);
uint32_t sys_timeouts_sleeptime(void);

struct SysTimeoutContext** sys_timeouts_get_next_timeout(void);
void lwip_cyclic_timer(uint8_t *arg);
