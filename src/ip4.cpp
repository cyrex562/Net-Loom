                           ...
                           const std::vector<NetworkInterface>& netifs)
{
    for (const auto& netif : netifs)
    {
        ...
        {
            const auto net = (dst.u32 & addr_info.netmask.u32);
...
can_forward_ip4_pkt(PacketContainer& pkt_buf)
{
    ...
    if (ip4_addr_experimental(dst_addr.u32))
    {
        return false;
    }
    if (ip4_addr_class_a(dst_addr.u32))
    {
 ...
 */
static NsStatus
forward_ip4_pkt(PacketContainer& pkt_buf, const std::vector<NetworkInterface>& netifs)
{
...
ip4_input_accept(NetworkInterface& netif)
{
    ...
    if (is_netif_up(netif) && !(out_ip4_addr_info.address.u32 == IP4_ADDR_ANY_U32))
    {
        /* unicast to this interface address? */
        if ((curr_dst_addr.address.u32 == out_ip4_addr_info.address) ||
            /* or broadcast on this interface network address? */
            netif_is_ip4_addr_bcast(curr_dst_addr.address, netif) ||
            (curr_dst_addr.address.u32) == pp_htonl(IP4_ADDR_LOOPBACK_U32))
        {
            return true;
...
ip4_input(PacketContainer& pkt_buf, NetworkInterface& netif, std::vector<NetworkInterface>& interfaces)
{
    ...
    size_t iphdr_len = ns_ntohs(get_ip4_hdr_len2(ip4_hdr_ptr));
    ...
    if (ip4_addr_is_mcast(curr_dst_addr.address))
    {
        ...
        if (netif.igmp_allowed && find_igmp_group(netif, curr_dst_addr, found_igmp_group))
        {
            ...
            if ((curr_dst_addr.address.u32 == allsystems) && (
                curr_src_addr.address.u32 == IP4_ADDR_ANY_U32))
            {
                check_ip_src = 0;
            }
        }
    ...
    else
    {
        ...
        if (!ip4_input_accept(netif))
        {
            ...
            if (!ip4_addr_is_loopback(curr_dst_addr.address))
            {
 ...
 */
NsStatus
ip4_output_if(PacketContainer& p,
 ...
 */
NsStatus
ip4_output_if_opt(struct PacketContainer* p,
                  const Ip4Addr* src,
                  const Ip4Addr* dest,
                  uint8_t ttl,
                  uint8_t tos,
 ...
 */
  NsStatus
  ip4_output_if_src(PacketContainer& p,
                    const Ip4AddrInfo& src,
                    const Ip4AddrInfo& dest,
                    uint8_t ttl,
                    uint8_t tos,
 ...
 */
NsStatus
ip4_output_if_opt_src(struct PacketContainer *p, const Ip4Addr *src, const Ip4Addr *dest,
    ...
    if (optlen != 0) {
      ...
      for (int i = 0; i < optlen_aligned / 2; i++) {
        chk_sum += ((uint16_t *)p->payload)[i];
          ...
          ns_assert("check that first PacketBuffer can hold struct Ip4Hdr",
          ...
          chk_sum += (&iphdr->dest.u32) & 0xFFFF;
          chk_sum += (&iphdr->dest.u32) >> 16;
          ...
          set_ip4_hdr_len(iphdr, ns_htons(p->tot_len));

          chk_sum += iphdr->_len;

          set_ip4_hdr_offset(iphdr, 0);
          set_ip4_hdr_id(iphdr, ns_htons(ip_id));
          ...
          chk_sum += (&iphdr->src.u32) & 0xFFFF;
          chk_sum += (&iphdr->src.u32) >> 16;
...
///
NsStatus
ip4_output_hinted(struct PacketContainer* pkt_buf,
                  const Ip4Addr* src,
                  const Ip4Addr* dest,
                  uint8_t ttl,
                  uint8_t tos,
                  ...
                  NetIfcHint* netif_hint)
{
    ...
    NsStatus err = ip4_output_if(pkt_buf, src, dest, ttl, tos, proto, netif);
