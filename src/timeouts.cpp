...
{
    ns_assert("Timeout time too long, max is LWIP_UINT32_MAX/4 msecs",
...

{
  ...
  if (timeout == nullptr) {
    ns_assert("sys_timeout: timeout != NULL, pool MEMP_SYS_TIMEOUT is empty", timeout != nullptr);
    return;
  }
...
sys_timeouts_sleeptime(void)
{
  ...
  if (TIME_LESS_THAN(next_timeout->time, now)) {
    ...
    ns_assert("invalid sleeptime", ret <= 0x7);
    return ret;
}
