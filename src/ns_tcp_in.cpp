...
#include "tcp_in.h"
 ...
 uint8_t flags;
 uint8_t recv_flags;
 struct PacketContainer* recv_data;
 ...
 */
void
tcp_input(struct PacketContainer* p, NetworkInterface* inp)
{
    ...
    ns_assert("tcp_input: invalid pbuf", p != nullptr);
    ...
    else
    {
        ...
        ns_assert("p->next != NULL", p->next != nullptr);
        ...
        ns_assert("p->len == 0", p->len == 0);
        ns_assert("p->tot_len == p->next->tot_len", p->tot_len == p->next->tot_len);
    } /* Convert fields in TCP header to host byte order. */
    tcphdr->src = ns_ntohs(tcphdr->src);
    tcphdr->dest = ns_ntohs(tcphdr->dest);
    seqno = tcphdr->seqno = ns_ntohl(tcphdr->seqno);
    ackno = tcphdr->ackno = ns_ntohl(tcphdr->ackno);
    tcphdr->wnd = ns_ntohs(tcphdr->wnd);
    ...
    {
        ns_assert("tcp_input: active pcb->state != CLOSED", pcb->state != CLOSED);
        ns_assert("tcp_input: active pcb->state != TIME-WAIT", pcb->state != TIME_WAIT);
        ns_assert("tcp_input: active pcb->state != LISTEN", pcb->state != LISTEN);
                ...
                curr_dst_addr))
        {
            ...
            ns_assert("tcp_input: pcb->next != pcb (before cache)", pcb->next != pcb);
            if (prev != nullptr)
            {
                prev->next = pcb->next;
            ...
            ns_assert("tcp_input: pcb->next != pcb (after cache)", pcb->next != pcb);
            break;
        }
        prev = pcb;
    }
    if (pcb == nullptr)
    {
        ...
        {
            ns_assert("tcp_input: TIME-WAIT pcb->state == TIME-WAIT",
                        pcb->state == TIME_WAIT);
            /* check if PCB is bound to specific netif */
            if ((pcb->netif_idx != NETIF_NO_INDEX) && (pcb->netif_idx != get_and_inc_netif_num(
                curr_netif)))
    ...
    if (pcb != nullptr)
    {
        ...
        if (err != ERR_ABRT)
        {
            ...
            {
                err = STATUS_SUCCESS; /* If the application has registered a "sent" function to be
                ...
                {
                    struct PacketContainer* rest = nullptr;
                    // pbuf_split_64k(recv_data, &rest);
                    ns_assert("pcb->refused_data == NULL",
    ...
    ns_assert("tcp_input: tcp_pcbs_sane()", tcp_pcbs_sane());
    return;
...
{
    ns_assert("tcp_input_delayed_close: invalid pcb", pcb != nullptr);
...
tcp_listen_input(struct TcpPcbListen* pcb)
{
    ...
    ns_assert("tcp_listen_input: invalid pcb", pcb != nullptr);
    ...
    else if (flags & TCP_SYN)
    {
        ...
        if (npcb == nullptr)
        {
            ...
            TCP_EVENT_ACCEPT(pcb, NULL, pcb->callback_arg, STATUS_E_MEM, err); /* err not useful here */
            return;
        }
        pcb->accepts_pending++;
        ...
        if (tcp_ext_arg_invoke_callbacks_passive_open(pcb, npcb) != STATUS_SUCCESS)
        {
            tcp_abandon(npcb, 0);
            return;
        } /* Send a SYN|ACK together with the MSS option. */
        NsStatus rc = tcp_enqueue_flags(npcb, TCP_SYN | TCP_ACK);
...
tcp_timewait_input(struct TcpPcb* pcb)
{
    ...
    ns_assert("tcp_timewait_input: invalid pcb", pcb != nullptr);
...
tcp_process(struct TcpPcb* pcb)
{
    ...
    ns_assert("tcp_process: invalid pcb", pcb != nullptr);
    ...
    if (flags & TCP_RST)
    {
        ...
        if (acceptable)
        {
            ...
            ns_assert("tcp_input: pcb->state != CLOSED", pcb->state != CLOSED);
            recv_flags |= TF_RESET;
    ...
    case SYN_SENT:
        Logf(true,
             "SYN-SENT: ackno %d pcb->snd_nxt %d unacked %d\n", ackno, pcb->snd_nxt,
                 ns_ntohl(pcb->unacked->tcphdr->seqno));
        ...
        if ((flags & TCP_ACK) && (flags & TCP_SYN) && (ackno == pcb->lastack + 1))
        {
            ...
            Logf(true,
                 "tcp_process (SENT): cwnd %d ssthresh %d\n", pcb->cwnd,
                     pcb->ssthresh);
            ns_assert("pcb->snd_queuelen > 0", (pcb->snd_queuelen > 0));
            --pcb->snd_queuelen;
            Logf(true,
            ...
            if (rseg == nullptr)
            {
                ...
                rseg = pcb->unsent;
                ns_assert("no segment to free", rseg != nullptr);
         ...
         * connected. */
            TCP_EVENT_CONNECTED(pcb, STATUS_SUCCESS, err);
            if (err == ERR_ABRT)
            {
                return ERR_ABRT;
            }
    ...
    case SYN_RCVD:
        if (flags & TCP_ACK)
        {
            ...
            if (TCP_SEQ_BETWEEN(ackno, pcb->lastack + 1, pcb->snd_nxt))
            {
                ...
                {
                    ns_assert("pcb->listener->accept != NULL",
                                pcb->listener->accept_fn != nullptr);
                    tcp_backlog_accepted(pcb); /* Call the accept function. */
                    TCP_EVENT_ACCEPT(pcb->listener, pcb, pcb->callback_arg, STATUS_SUCCESS, err);
                }
                if (err != STATUS_SUCCESS)
                {
...
{
    ns_assert("tcp_oos_insert_segment: invalid cseg", cseg != nullptr);
                        ...
                        struct TcpSeg* dbg_other_seg_list)
{
    while (seg_list != nullptr && TCP_SEQ_LEQ(
        ns_ntohl(seg_list->tcphdr->seqno) + tcp_tcplen(seg_list),
        ackno))
    {
        Logf(true,
             ("tcp_receive: removing %d:%d from pcb->%s\n",
                 ns_ntohl(seg_list->tcphdr->seqno), ns_ntohl(seg_list->tcphdr->seqno)
        ...
        ns_assert("pcb->snd_queuelen >= pbuf_clen(next->p)",
        ...
        {
            ns_assert("tcp_receive: valid queue length",
...
{
    int found_dupack = 0;
    ns_assert("tcp_receive: invalid pcb", pcb != nullptr);
    ns_assert("tcp_receive: wrong state", pcb->state >= ESTABLISHED);
    if (flags & TCP_ACK)
    {
        ...
        else if (TCP_SEQ_BETWEEN(ackno, pcb->lastack + 1, pcb->snd_nxt))
        {
         ...
         ssthresh). */
            ...
            Logf(true,
                 ...
                 ackno,
                 pcb->unacked != nullptr ? ns_ntohl(pcb->unacked->tcphdr->seqno) : 0,
                 pcb->unacked != nullptr
                     ? ns_ntohl(pcb->unacked->tcphdr->seqno) + tcp_tcplen(pcb->unacked)
                     : 0);
            ...
            if (pcb->flags & TF_RTO)
            {
                ...
                if (pcb->unacked == nullptr)
                {
                    if ((pcb->unsent == nullptr) || (TCP_SEQ_LEQ(
                        pcb->rto_end,
                        ns_ntohl(pcb->unsent->tcphdr->seqno))))
                    {
       ...
       segment is larger than rcv_nxt. */ /*    if (tcp_seq_lt(seqno, pcb->rcv_nxt)) {
        ...
        if (TCP_SEQ_BETWEEN(pcb->rcv_nxt, seqno + 1, seqno + tcplen - 1))
        {
            /* Trimming the first edge is done by pushing the payload
               ...
               length.*/
            struct PacketContainer* p = inseg.p;
            uint32_t off32 = pcb->rcv_nxt - seqno;
            ns_assert("inseg.p != NULL", inseg.p);
            ns_assert("insane offset!", (off32 < 0xffff));
            uint16_t off = (uint16_t)off32;
            ns_assert("PacketBuffer too short!", (((int32_t)inseg.p->tot_len) >= off));
            inseg.len -= off;
        ...
        if (TCP_SEQ_BETWEEN(seqno, pcb->rcv_nxt, pcb->rcv_nxt + pcb->rcv_wnd - 1))
        {
            if (pcb->rcv_nxt == seqno)
            {
                ...
                if (tcplen > pcb->rcv_wnd)
                {
                    ...
                    ns_assert("window size > 0xFFFF", (pcb->rcv_wnd) <= 0xFFFF);
                    ...
                    tcplen = tcp_tcplen(&inseg);
                    ns_assert("tcp_receive: segment not trimmed correctly to rcv_wnd\n",
           ...
           - inseq overlaps with ooseq */
             ...
             * segment on ooseq */
                ...
                ns_assert("tcp_receive: tcplen > rcv_wnd\n", pcb->rcv_wnd >= tcplen);
                pcb->rcv_wnd -= tcplen;
           ...
           are now in sequence. */
                while (pcb->ooseq != nullptr && pcb->ooseq->tcphdr->seqno == pcb->rcv_nxt)
                {
                    ...
                    ns_assert("tcp_receive: ooseq tcplen > rcv_wnd\n",
            ...
            else
            {
                ...
                }
                {
                    ...
                    for (struct TcpSeg* next = pcb->ooseq; next != nullptr; prev = next,
                         next = next->next)
                    {
                        struct PacketContainer* p = next->p;
                        int stop_here = 0;
...
tcp_parseopt(struct TcpPcb* pcb)
{
    ...
    uint16_t mss;
    uint32_t tsval;
    ns_assert("tcp_parseopt: invalid pcb", pcb != nullptr);
        ...
        for (tcp_optidx = 0; tcp_optidx < tcphdr_optlen;)
        {
            ...
            case LWIP_TCP_OPT_WS:
                ...
                if ((flags & TCP_SYN) && !(pcb->flags & TF_WND_SCALE))
                {
                    ...
                    ns_assert("window not at default value",
