...
/* This treats "PacketBuffer pools" just like any other pool.
 * Allocates buffers for a PacketBuffer struct AND a payload size */
#define LWIP_PBUF_MEMPOOL(name, num, payload, desc) LWIP_MEMPOOL(name, num, (LWIP_MEM_ALIGN_SIZE(sizeof(struct PacketBuffer)) + LWIP_MEM_ALIGN_SIZE(payload)), desc)
