...
/// @return host order (!) ns checksum (non-inverted Internet sum)
///
...
static uint16_t inet_cksum_pseudo_base(struct PacketContainer* p,
                                       uint8_t proto,
                                       size_t proto_len,
                                       uint32_t acc)
{
    int swapped = 0; /* iterate through all PacketBuffer in chain */
    for (struct PacketContainer* q = p; q != nullptr; q = q->next)
    {
        // Logf(INET_DEBUG,
    ...
    acc += (uint32_t)ns_htons((uint16_t)proto);
    acc += (uint32_t)ns_htons(proto_len); /* Fold 32-bit sum to 16 bits
     calling this twice is probably faster than if statements... */
    acc = fold_u32(acc);
 ...
 */
uint16_t inet_chksum_pseudo(PacketContainer& p,
                            uint8_t proto,
                            uint16_t proto_len,
                            const Ip4Addr& src,
                            const Ip4Addr& dest)
 ...
 * @return checksum (as uint16_t) to be saved directly in the protocol header
 */
uint16_t ip6_chksum_pseudo(struct PacketContainer& p,
                           uint8_t proto,
                           size_t proto_len,
                           const Ip6Addr& src,
                           const Ip6Addr& dest)
 ...
 * @return checksum (as uint16_t) to be saved directly in the protocol header
 */
uint16_t ip_chksum_pseudo(PacketContainer& p,
                          uint8_t proto,
                          uint16_t proto_len,
...
static uint16_t inet_cksum_pseudo_partial_base(struct PacketContainer* p,
                                               uint8_t proto,
                                               uint16_t proto_len,
                                               uint16_t chksum_len,
                                               uint32_t acc)
{
    int swapped = 0;
    for (struct PacketContainer* q = p; (q != nullptr) && (chksum_len > 0); q = q->next)
    {
        // Logf(INET_DEBUG,
        //      ("inet_chksum_pseudo(): checksumming PacketBuffer %p (has next %p) \n", (void
        //          *)q, (uint8_t *)q->next));
        ...
        ns_assert("delete me", chksum_len < 0x7fff);
        /*Logf(INET_DEBUG, ("inet_chksum_pseudo(): unwrapped lwip_standard_checksum()=%x
 ...
 * @return checksum (as uint16_t) to be saved directly in the protocol header
 */
uint16_t inet_chksum_pseudo_partial(PacketContainer& p,
                                    uint8_t proto,
                                    uint16_t proto_len,
                                    uint16_t chksum_len,
                                    const Ip4Addr& src,
 ...
 * @return checksum (as uint16_t) to be saved directly in the protocol header
 */
uint16_t ip6_chksum_pseudo_partial(PacketContainer& p,
                                   uint8_t proto,
                                   size_t proto_len,
                                   size_t chksum_len,
                                   const Ip6Addr& src,
 ...
 */
uint16_t
ip_chksum_pseudo_partial(PacketContainer& p,
                         uint8_t proto,
                         size_t proto_len,
                         size_t chksum_len,
                         const IpAddrInfo& src,
...
/// @return checksum (as uint16_t) to be saved directly in the protocol header
///
uint16_t inet_chksum_pbuf(struct PacketContainer* p)
{
    auto swapped = false;
    uint32_t acc = 0;
