...
struct DhcpContext
{
    ...
    bool pcb_allocated; /** current DHCP state machine state */
    DhcpState state; /** retries of current request */
    uint32_t tries;
    ...
    uint64_t t2_timeout; /* #ticks with period DHCP_COARSE_TIMER_SECS for rebind time */
    ...
    uint64_t t0_timeout; /* #ticks with period DHCP_COARSE_TIMER_SECS for lease time */
