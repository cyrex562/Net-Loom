...
int file_write_http_header(FILE *data_file, const char *filename, int file_size, uint16_t *http_hdr_len,
                           uint16_t *http_hdr_chksum, u8_t provide_content_len, int is_compressed);
...
int main(int argc, char *argv[])
{
  ...
  LWIP_ASSERT("sizeof(hdr_buf) must fit into an uint16_t", sizeof(hdr_buf) <= 0xffff);
  ...
  fprintf(data_file, "#include \"apps/fs.h\"" NEWLINE);
  fprintf(data_file, "#include \"def.h\"" NEWLINE NEWLINE NEWLINE);
...
static int write_checksums(FILE *struct_file, const char *varname,
                           uint16_t hdr_len, uint16_t hdr_chksum, const u8_t *file_data, size_t file_size)
{
  ...
  for (offset = hdr_len; ; offset += len) {
    ...
    chksum = ~inet_chksum(data, (uint16_t)len);
                           ...
                           uint16_t *http_hdr_chksum, u8_t provide_content_len, int is_compressed)
{
  ...
  if (precalcChksum) {
    ...
    acc = ~inet_chksum(hdr_buf, (uint16_t)hdr_len);
    *http_hdr_len = (uint16_t)hdr_len;
    *http_hdr_chksum = acc;
  }
